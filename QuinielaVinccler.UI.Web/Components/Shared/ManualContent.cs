...
/// <summary>
/// Contenido del manual de usuario embebido como strings constantes.
/// Dos versiones: solo usuario común, y completo (común + admin).
/// </summary>
public static class ManualContent
{
    public const string Comun = """
...
Botón rojo arriba a la derecha. **Borra todo:** puntos, resultados, posiciones finales, marcadores exactos y puntajes totales.

Pide confirmación antes de ejecutar. **No se puede deshacer.** Úsalo solo si necesitas empezar desde cero.
...
- 🟢 **ABIERTA** — sin restricciones activas.
- 🟡 **ABIERTA — cierre programado** — muestra fecha y tiempo restante.
- 🔴 **CERRADA** — por fecha pasada o por cierre manual.

### Cierre manual

Toggle que fuerza el cierre **inmediatamente**, sin importar la fecha programada.
...
- Los usuarios solo pueden **ver** sus planillas en modo lectura.
...
1. Generar lotes de planillas en `/admin → Generar Planillas`.
2. Imprimir los PDFs y repartir las planillas físicas.
3. Configurar la fecha de cierre en `/admin → Configuración`.

### Durante el torneo

1. Cada vez que termina un partido, entrar a `/admin → Resultados`.
2. Seleccionar el resultado y guardar.
3. Los puntos se **recalculan automáticamente**.

### Al final del torneo

1. Registrar los **marcadores exactos** de semifinales y final.
2. Registrar las **posiciones finales** y los equipos extras.
